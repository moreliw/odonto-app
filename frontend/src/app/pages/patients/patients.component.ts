            ...
            <tbody>
              @if (loading) {
                ...
                @for (p of pagedPatients; track p.id) {
                  <tr class="patient-clickable-row" tabindex="0" (click)="openPatient(p)" (keydown.enter)="openPatient(p)" (keydown.space)="$event.preventDefault(); openPatient(p)">
                    ...
                    <td class="patients-actions-cell">
                      <div class="table-actions">
                        <button class="btn btn-sm btn-ghost" (click)="$event.stopPropagation(); openEdit(p)" title="Editar">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                        </button>
                        <button class="btn btn-sm btn-ghost" style="color:var(--danger);" (click)="$event.stopPropagation(); confirmDelete(p)" title="Excluir">
